...
function getSrgbArray(srgbaString: String) {
  srgbaString = String(srgbaString).replace("rgba(", "");
  srgbaString = String(srgbaString).replace(")", "");
  // @ts-ignore
  let newData: Array<Number> = srgbaString.split(",");
...
function srgbaToHsv(r: number, g: number, b: number) {
  (r /= 255), (g /= 255), (b /= 255);
  ...
  private onEyedropperEvent(event?: Event): void {
    if (window.EyeDropper) {
      ...
      eyeDropper.open().then((result) => {
        // @ts-ignore
        let srgbArray: Array = getSrgbArray(result.sRGBHex);
        let hsvArray = srgbaToHsv(srgbArray[0], srgbArray[1], srgbArray[2]);

        let finalHEX = FieldColourHsvSliders.hsvToHex(
          hsvArray[0],
          hsvArray[1],
          hsvArray[2]
        );

        this.setValue(finalHEX);
        this.updateSliderValues();
      });
    }
  }
